...
const Payment = () => {
  ...
  const handleSendOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/payments/send-payment-otp', {
        method: 'POST',
        headers: {
  ...
  const handleVerifyOtp = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch('http://localhost:8000/api/payments/verify-payment-otp', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
