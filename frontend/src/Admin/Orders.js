...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await axios.get('https://gshankarwenli-1x.onrender.com/api/payments');
  ...
  const handlePaymentConfirmation = async (orderId) => {
    try {
      await axios.post(`https://gshankarwenli-1x.onrender.com/api/payments/${orderId}/confirm`);
